...
public class NotificationShellCmd extends ShellCommand {
    ...
    private static final String USAGE = "usage: cmd notification SUBCMD [args]\n\n"
            ...
            + "  perform_action <notification-key> <action> [content|dismiss|reply|mute]\n"
    ...
    public int onCommand(String cmd) {
        ...
        try {
            switch (cmd.replace('-', '_')) {
                ...
                case "perform_action": {
                    String key = getNextArgRequired();
                    String actionStr = getNextArgRequired();
                    int action;
                    switch (actionStr) {
                        case "content": action = NotificationManager.ACTION_CONTENT; break;
                        case "dismiss": action = NotificationManager.ACTION_DISMISS; break;
                        case "reply":  action = NotificationManager.ACTION_REPLY;  break;
                        case "mute":   action = NotificationManager.ACTION_MUTE;   break;
                        default:
                            pw.println("error: invalid action: " + actionStr
                                    + " (expected: content|dismiss|reply|mute)");
                            return 1;
                    }
                    pw.println("performing action " + actionStr + " on key=" + key);
                    mBinderService.performNotificationAction(key, action, null);
                    pw.println("done");
                    break;
                }
                default:
